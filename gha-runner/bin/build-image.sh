...
function cleanup() {
  exit_code=$?
  if [[ $(docker buildx ls --format=json | jq -r '.Name' | grep build-gha-runner-${RUN_ID:-} | wc -l) -ne 0 ]]; then 
      docker buildx rm build-gha-runner-${RUN_ID:-}
  fi
...
if [ -z "$local_build" ]; then
  ...
  while (( 1 == 1 ))
  do
    docker buildx create --bootstrap --name=build-gha-runner-$RUN_ID --driver=kubernetes \
  ...
  set -e
  docker buildx use build-gha-runner-$RUN_ID
else
...
docker buildx build --platform linux/amd64 . -f Dockerfile --build-arg="VAULT_VERSION=$VAULT_VERSION" \
             --push -t paulcarltondae/runner:$VERSION \

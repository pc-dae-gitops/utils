...
# Setup dae access to the cluster

export cluster_name
export hostname
export cluster_name="kind-${hostname}-${cluster_name}"
git pull
export type=kind
...
if [[ `git status --porcelain` ]]; then
  git commit -m "deploy kustomizations to apply dae SA to kind cluster $hostname-$cluster_name"
  git pull
  git push
fi
...
# Wait for sa kustomization to be created
echo "Waiting for sa kustomization to be created"
while ( true ); do
  set +e
  kubectl get kustomization/sa -n flux-system 2>/dev/null
  ret=$?
  set -e
  if [ "$ret" == "0" ]; then
    break
...
echo "Waiting for sa to be applied"
kubectl wait --timeout=5m --for=condition=Ready kustomization/sa -n flux-system

git pull
mkdir -p clusters//kind/$hostname-$cluster_name/dae
cat $(local_or_global resources/leaf-dae.yaml)| envsubst > clusters/kind/$hostname-$cluster_name/dae/flux.yaml
git add clusters/kind/$hostname-$cluster_name/dae/flux.yaml
...
# Setup dae access to the cluster using the dae SA

export token="$(kubectl --kubeconfig ~/.kube/$location-${cluster_name}.kubeconfig get secrets -n dae -l "weave.works/sa=dae" -o jsonpath={.items[0].data.token} | base64 -d)"
...
cat $(local_or_global resources/kubeconfig.yaml) | envsubst > /tmp/kind-${hostname}-${cluster_name}-kubeconfig.yaml
vault kv put -mount=secrets/leaf-cluster-secrets kind-${hostname}-${cluster_name}  value.yaml="$(cat /tmp/kind-${hostname}-${cluster_name}-kubeconfig.yaml)"
